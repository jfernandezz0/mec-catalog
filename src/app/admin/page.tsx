...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'catalog' | 'create' | 'edit' | 'categories' | 'import' | 'config' | 'sales' | 'sales-create' | 'analytics' | 'generate_list'>('catalog');
  
  // PDF list generation states
  ...
  function handleTabChange(tab: 'catalog' | 'create' | 'categories' | 'import' | 'config' | 'sales' | 'sales-create' | 'analytics' | 'generate_list') {
    resetForm();
    setActiveTab(tab);
...
ALTER TABLE categories ADD COLUMN IF NOT EXISTS discount_percent INTEGER CHECK (discount_percent >= 1 AND discount_percent <= 100);`}
        ...
        {activeTab === 'generate_list' && (() => {
          // Sort articles by ID ascending
          const sortedArticles = [...articles].sort((a, b) => a.id - b.id);
          
          // Filter articles based on stock filter
      ...
      <div style={{ padding: '0px', fontFamily: 'system-ui, sans-serif' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', borderBottom: '2px solid #000', paddingBottom: '8px', marginBottom: '16px' }}>
          ...
          <div style={{ textAlign: 'right', fontSize: '10px', color: '#444' }}>
            Filtro: {listStockFilter === 'stock' ? 'Solo con stock' : listStockFilter === 'sin_stock' ? 'Solo sin stock' : 'Todos'} &middot; Total: {articles.filter(article => {
              if (listStockFilter === 'stock') return article.quantity > 0;
              if (listStockFilter === 'sin_stock') return article.quantity === 0;
              return true;
            }).length} arts.
          </div>
        </div>
